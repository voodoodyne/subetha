...
public class ReceiptCommand extends BaseCommand
{
	...
	public String execute(String commandString, Session session)
	{
		...
		{
			return "501 Syntax: RCPT TO: <address>  Error in parameters: \""
					+ getArgPredicate(commandString) + "\"";
		}
		else
		{
			final String recipientAddress = extractEmailAddress(args, 3);
			if (session.isRecipientDomainFilteringEnabled()
					&& !canAcceptMailToDomain(recipientAddress))
			{
	...
	{
		return commandDispatcher.getServerContext().accept(session.getSender(),
				recipientAddress);
	}
